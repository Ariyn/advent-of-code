...
const val maximumFilesystemSize = 70000000
const val requiredSizeForUpdate = 30000000
...
class Parser(private var lines: List<String>) {
    ...
    private fun parseLine(l: String) {
...
//    val lines = exampleLines
    ...
    val requiredToRemoveSize = requiredSizeForUpdate - (maximumFilesystemSize - parser.rootDirectory.size())

    val directorySizes =
            parser.entireDirectories.map { it.size() }
    val filteredDirectories = directorySizes.filter { size -> requiredToRemoveSize <= size }
    val minSize = filteredDirectories.min()
