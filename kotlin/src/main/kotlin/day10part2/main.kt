...
class CPU(
    ...
    private var crtRow = ""
    val crtRows = mutableListOf<String>()
    ...
    fun run() {
        ...
        while (i < lines.size) {
            ...
            crtRow += if (isTouchingDrawing(crtRow, registers["X"]!!)) {
                "#"
            } else {
                " "
            }
    ...
    fun isTouchingDrawing(crtRow: String, x: Int): Boolean {
        return x == crtRow.length + 1 || x + 1 == crtRow.length + 1 || x + 2 == crtRow.length + 1
    }
}
...
//    lines = exampleLines
    ...
    cpu.run()
    for (i in cpu.crtRows.indices) {
        println(cpu.crtRows[i])
    }
}
